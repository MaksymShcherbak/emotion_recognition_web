function PredictionPlot({ predictions, onExport }) {
  const maxLabel =
    predictions != null
      ? predictions.reduce((max, obj) =>
          obj.probability > max.probability ? obj : max,
        ).label
      : null;
      ...
      {predictions != null && (
        <>
          <div className="predictions-title">
            <p>
              Predicted Emotion: <span>{maxLabel}</span>
            </p>
            {predictions && onExport ? (
          ...
          <div className="results">
            {predictions.map(({ label, probability }) => (
              <div key={label}>
                <span
                  className={
                    maxLabel === label ? "results-bar selected" : "results-bar"
                  }
                  style={{ height: `${probability * 163}px` }}
                ></span>
